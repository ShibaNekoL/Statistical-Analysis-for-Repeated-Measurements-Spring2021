...
1  3 A M 23 1 1 1 1 1    /* active treatment */
...
2 55 A M 31 1 1 1 1 1   /* wide data */
; 
run; 
...
proc genmod data=resp descend; 
   ...
   repeated subject=id(center) / type=exch corrw covb; 
run; 

proc genmod data=resp descend; 
   class id treatment(ref="P") center(ref="1") sex(ref="M") baseline(ref="0") / param=ref; 
   model outcome=treatment center sex age baseline / dist=binomial; 
   repeated subject=id(center) / type=un corrw covb; 
run; 

proc genmod data=resp descend; 
   class id treatment(ref="P") center(ref="1") sex(ref="M") baseline(ref="0") / param=ref; 
   model outcome=treatment center sex age baseline / dist=binomial; 
   repeated subject=id(center) / type=ar(1) corrw covb; 
run; 

proc genmod data=resp descend; 
   class id treatment(ref="P") center(ref="1") sex(ref="M") baseline(ref="0") / param=ref; 
   model outcome=treatment center sex age baseline / dist=binomial; 
   repeated subject=id(center) / type=ind corrw covb; 
run; 

proc genmod data=resp descend; 
   class id treatment(ref="P") center(ref="1") sex(ref="M") baseline(ref="0") /param=ref; 
   model outcome=treatment center sex age baseline / dist=bin; 
   repeated subject=id(center) / corrw covb type=USER( 1.0, 0.3, 0.2, 0.3,
  0.3, 1.0, 0.4, 0.3,
  0.2, 0.4, 1.0, 0.4,
  0.3, 0.3, 0.4, 1.0 );
run;
...
proc genmod data=resp descend; 
   class id treatment(ref="P") center(ref="1") sex(ref="M") baseline(ref="0") /param=ref; 
   model outcome=treatment center sex age baseline / dist=bin; 
   repeated subject=id(center) / corrw covb type=USER( 1.0, 0.9, 0.8, 0.6,
  0.9, 1.0, 0.9, 0.8,
  0.8, 0.9, 1.0, 0.9,
  0.6, 0.8, 0.9, 1.0 );
run;
