...
/**********************************************************************************/
/* Logistic regression without considering repeated measurements */
proc logistic data=resp plots(only)=(roc) rocoptions(id=prob) descend; 
   class id treatment center sex; 
   model outcome=treatment center sex age baseline; 
run; 

/* prob = predicted probability of Y = 1 (good condition) */
/* sensitivity = Pr( Y^ = 1 | Y = 1)  */
/* specificity = Pr( Y^ = 0 | Y = 0)  */


/* First-order Markov model */
/* Create data for transition model */
data respTran1; 
   set resp;
   if visit=1 then do
		outcome=visit1;
...
/* Transition model */
proc logistic data=respTran1 plots(only)=(roc) rocoptions(id=prob) descend; 
   class id treatment center sex; 
   model outcome=treatment center sex age preoutcome; 
run;

/* Check if the correlation is 0 */
proc glimmix data=respTran1 empirical; 
   class id treatment center sex baseline; 
   model outcome=treatment center sex age preoutcome / dist=binomial s; 
   random residual / subject=id(center) type=cs vcorr;
   covtest INDEP;
run;

/* H0: error terms are independent to each other */
/* H1: error terms are NOT independent to each other */
...
/* Transition model */
proc logistic data=respTran2 plots(only)=(roc) rocoptions(id=prob) descend; 
   class id treatment center sex; 
   model outcome=treatment center sex age preoutcome preoutcome2; 
run;

/* Check if the correlation is 0 */
proc glimmix data=respTran2 empirical; 
   class id treatment center sex baseline; 
   model outcome=treatment center sex age preoutcome preoutcome2 / dist=binomial s; 
   random residual / subject=id(center) type=cs vcorr;
   covtest INDEP;
run;
...
proc genmod data=resp descend; 
   class id treatment(ref="P") center(ref="1") sex(ref="M") baseline(ref="0") / param=ref; 
   model outcome=treatment center sex age baseline / dist=binomial; 
   repeated subject=id(center) / corr=exch; 
   output out=prediction pred=predi;
run;

/* pred = predicted probability of Y = 1 (good condition) */

proc logistic data=prediction plots(only)=(roc) rocoptions(id=prob) descend; 
   model outcome=predi; 
run; 
...
/* GLMM --- G-side */
proc glimmix data=resp empirical; 
   class id treatment(ref="P") center(ref="1") sex(ref="M") baseline(ref="0"); 
   model outcome=treatment center sex age baseline / dist=binomial s; 
